...
class WindowsPloneInstaller:
    ...
    def __init__(self):
        ...
        self.installerPath = os.path.realpath(__file__).split(".")[0]+".exe"

        self.ploneKey = r'SOFTWARE\PloneInstaller' #our Windows registry key under HKEY_CURRENT_USER
        self.runOnceKey = r'Software\Microsoft\Windows\CurrentVersion\RunOnce'

        self.requiredBuildNumber = 15063
    ...
    def initGUI(self):
        ...
        if self.install_status == "wsl_enabled":
            self.statusText.set('Picking up where we left off. Installing Linux Subsystem...')
            self.runPS("./PS/installWSL.ps1") #Install Ubuntu on Windows
            self.waitForStatusChange(5000) # Not sure how long this takes

            if self.install_status == "wsl_installed":
                self.runPS("./PS/installPlone.ps1") #Install Plone on the new instance of WSL

        elif self.install_status == "begin":
            self.runPS("./PS/getWinInfo.ps1")
            self.waitForStatusChange(10)

            if self.install_status == "got_win_info":
                k = OpenKey(HKEY_CURRENT_USER, self.ploneKey)
                
                envWinBuildNumber = int(str(QueryValueEx(k, "win_version")).split('.')[2].split("'")[0]) #this feels 'rigged.' Moved it to PowerShell because it's much more reliable, however.

                if envWinBuildNumber >= self.requiredBuildNumber:
    ...
    def initInstall(self, event):
        ...
        if self.install_type.get(): #if this is true, this machine has proper version for WSL route
            ...
            SetValue(HKEY_CURRENT_USER, self.runOnceKey, REG_SZ, self.installerPath) #Set Win Registry to load our installer after the next restart
            self.runPS("./PS/enableWSL.ps1") #Make sure WSL is enabled and check if it is already installed
            self.waitForStatusChange(15)
